///
/// HybridMmfileSpecSwift.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © 2025 Marc Rousavy @ Margelo
///

#pragma once

#include "HybridMmfileSpec.hpp"

// Forward declaration of `HybridMmfileSpec_cxx` to properly resolve imports.
namespace NitroMmfile { class HybridMmfileSpec_cxx; }

// Forward declaration of `ArrayBuffer` to properly resolve imports.
namespace NitroModules { class ArrayBuffer; }
// Forward declaration of `ArrayBufferHolder` to properly resolve imports.
namespace NitroModules { class ArrayBufferHolder; }

#include <NitroModules/ArrayBuffer.hpp>
#include <NitroModules/ArrayBufferHolder.hpp>

#include "NitroMmfile-Swift-Cxx-Umbrella.hpp"

namespace margelo::nitro::mmfile {

  /**
   * The C++ part of HybridMmfileSpec_cxx.swift.
   *
   * HybridMmfileSpecSwift (C++) accesses HybridMmfileSpec_cxx (Swift), and might
   * contain some additional bridging code for C++ <> Swift interop.
   *
   * Since this obviously introduces an overhead, I hope at some point in
   * the future, HybridMmfileSpec_cxx can directly inherit from the C++ class HybridMmfileSpec
   * to simplify the whole structure and memory management.
   */
  class HybridMmfileSpecSwift: public virtual HybridMmfileSpec {
  public:
    // Constructor from a Swift instance
    explicit HybridMmfileSpecSwift(const NitroMmfile::HybridMmfileSpec_cxx& swiftPart):
      HybridObject(HybridMmfileSpec::TAG),
      _swiftPart(swiftPart) { }

  public:
    // Get the Swift part
    inline NitroMmfile::HybridMmfileSpec_cxx& getSwiftPart() noexcept {
      return _swiftPart;
    }

  public:
    // Get memory pressure
    inline size_t getExternalMemorySize() noexcept override {
      return _swiftPart.getMemorySize();
    }

  public:
    // Properties
    inline double getSize() noexcept override {
      return _swiftPart.getSize();
    }

  public:
    // Methods
    inline void resize(double newSize) override {
      auto __result = _swiftPart.resize(std::forward<decltype(newSize)>(newSize));
      if (__result.hasError()) [[unlikely]] {
        std::rethrow_exception(__result.error());
      }
    }
    inline void clear() override {
      auto __result = _swiftPart.clear();
      if (__result.hasError()) [[unlikely]] {
        std::rethrow_exception(__result.error());
      }
    }
    inline void append(const std::shared_ptr<ArrayBuffer>& data) override {
      auto __result = _swiftPart.append(ArrayBufferHolder(data));
      if (__result.hasError()) [[unlikely]] {
        std::rethrow_exception(__result.error());
      }
    }
    inline void write(double offset, const std::shared_ptr<ArrayBuffer>& data) override {
      auto __result = _swiftPart.write(std::forward<decltype(offset)>(offset), ArrayBufferHolder(data));
      if (__result.hasError()) [[unlikely]] {
        std::rethrow_exception(__result.error());
      }
    }
    inline double read(double offset, const std::shared_ptr<ArrayBuffer>& data) override {
      auto __result = _swiftPart.read(std::forward<decltype(offset)>(offset), ArrayBufferHolder(data));
      if (__result.hasError()) [[unlikely]] {
        std::rethrow_exception(__result.error());
      }
      auto __value = std::move(__result.value());
      return __value;
    }

  private:
    NitroMmfile::HybridMmfileSpec_cxx _swiftPart;
  };

} // namespace margelo::nitro::mmfile
